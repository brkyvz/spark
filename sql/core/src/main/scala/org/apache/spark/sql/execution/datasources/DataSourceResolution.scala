...
import org.apache.spark.sql.catalyst.plans.logical.{CreateTableAsSelect, CreateV2Table, DescribeTable, DropTable, LogicalPlan, ReplaceTable, ReplaceTableAsSelect}
...
import org.apache.spark.sql.execution.datasources.v2.{CatalogTableAsV2, DataSourceV2Relation, V1RelationProvider, V2CatalogCompatibleV1Source}
...
case class DataSourceResolution(
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan resolveOperators {
    ...
    case DescribeTableStatement(CatalogObjectIdentifier(maybeCatalog, ident),
        partitionSpec, isExtended) if partitionSpec.isEmpty =>
      if (partitionSpec.nonEmpty) {
        throw new AnalysisException("DESCRIBE TABLE... PARTITION is not supported for v2 tables.")
      }
      DescribeTable(maybeCatalog.getOrElse(v2SessionCatalog).asTableCatalog, ident, isExtended)
    ...
    case replace: ReplaceTableStatement =>
      ...
      val catalog = maybeCatalog.getOrElse(v2SessionCatalog).asTableCatalog
      ...
      if (v1WriteOverrideSet.contains(provider.toLowerCase(Locale.ROOT))) {
        ...
        provider match {
          case _ if classOf[TableProvider].isAssignableFrom(providerClass) =>
            None
          case _ if classOf[V2CatalogCompatibleV1Source].isAssignableFrom(providerClass) =>
            None
          case _ =>
            Some(provider)
        }
